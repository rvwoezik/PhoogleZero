...
using PhoogleZero.Tests.TestDatas;
...
namespace PhoogleZero.Tests
{
    ...
    /// Provides methods to easily work with <see cref="PhoogleZeroDbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase<PhoogleZeroTestModule>
    {
        ...
        {
            void NormalizeDbContext(PhoogleZeroDbContext context)
            {
        ...
        protected void UsingDbContext(Action<PhoogleZeroDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<PhoogleZeroDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<PhoogleZeroDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<PhoogleZeroDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<PhoogleZeroDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PhoogleZeroDbContext>())
                {
                    action(context);
                    context.SaveChanges();
                }
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<PhoogleZeroDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PhoogleZeroDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<PhoogleZeroDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<PhoogleZeroDbContext, Task<T>> func)
        {
            T result;
